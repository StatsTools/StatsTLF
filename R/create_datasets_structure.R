...
#'
#' This function creates a package structure folder for a specified package name within the '03_Algorithm' directory. It sets up necessary template files and directories for the package.
#'
#' @param pkg_name A character string specifying the name of the package.
#'
#' @return The path to the created package structure folder.
#' @export
#'
#' @examples
#' \dontrun{
#' # Example usage:
#' # Create a package structure for 'Teste'
#' create_datasets_structure('Teste')
#' }
create_datasets_structure <- function(pkg_name) {
  ...
  file.copy(template_path, paste0(path, '\\caller_datasets.R'))
  file.rename(paste0(path, '\\caller_datasets.R'), paste0(path, "\\caller_", pkg_name, '.R'))
